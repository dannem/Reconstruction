function hh=gimage(cases,line_handle)
%gimage  Labels plotted points with their case names or case number.
%   gimage(CASES) displays the graph window, puts up a cross-hair, and
...
%   a name.  CASES can also be any grouping variable, which gimage converts
%   to labels.
%
%   gimage with no arguments labels each case with its case number.  It
...
%   
%   HH = gimage(CASES,LINE_HANDLE) returns a vector of handles
...
[az el] = view;
if az ~= 0 || el ~= 90
   error(message('stats:gimage:BadView'));
end
if (nargin < 1)
    cases = [];
...
if (~isempty(hManager.CurrentMode))
    error(message('stats:gimage:ModeActive', get( hManager.CurrentMode, 'name' )))
end

a = findobj(figh, 'Type', 'axes');
if (length(a) < 2)
   h=gimagesub(dolengthwng,cases,line_handle);
else
   h = [];
   bigax = gca;
   ...
   while ~isempty(x0)
      ...
      [h0,dolengthwng] = gimagesub(dolengthwng,cases,line_handle,x0,y0,x1,y1);
      h = [h; h0(:)];
...
patches = [];
scatters = [];
if 0%(~matlab.graphics.internal.isGraphicsVersion1())
...
elseif (nlines == 0)
    ...
    if ~matlab.graphics.internal.isGraphicsVersion1()
        if isempty(scatters)
            ...
            % no lines is valid for gimage().
            error(message('stats:gimage:NoLineHGUsingMATLABClasses'));
        end
        xdat = [];
        ydat = [];
        ...
        if (isempty(patches))
            error(message('stats:gimage:NoLine'));
        end
...
while(~isempty(x0))
   ...
   if ~isempty(idx)               % if any points were selected
      ...
      if (c0 < 0)
         ...
         if dolengthwng
             dolengthwng = false;
             warning(message('stats:gimage:BadLength'));
         end
      end
      ...
      if adding
         ...
         xp=(xdat(idx)-min(xdat))/(max(xdat)-min(xdat));
         yp=(ydat(idx)-min(ydat))/(max(ydat)-min(ydat));
         if xp>0.9
             xp=0.9;
         elseif xp<0
             xp=0;
         ...
         axes('pos',[xp*0.9 yp*0.9 .1 .1])
         imshow(ims{idx})
         set(gca,'Ydir','reverse')
         colormap('gray');      
         % Regular selection, add label to selected points
         h0 = text(x0, y0, t0, 'VerticalAlignment', 'baseline', 'Tag','gimage');
         h = [h; h0(:)];
         f = [];
         if (~rectangular), f = find(d-d1 <= 1e-2*d1); end
         if (length(f) > 1)
            disp(getString(message('stats:gimage:MultipleObs')));
         ...
         h0 = findobj(axesh,'Type','text','Tag','gimage');
         hrem = [];
         for k=1:length(x0)
